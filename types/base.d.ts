...
export type ScopedSlots<T> = { [K in StringKeyOf<T>]: (props: T[K]) => VNodeChildrenArrayContents | string } & {
    [name: string]: (props: any) => VNodeChildrenArrayContents | string;
};

export type EventHandlers<E> = { [K in StringKeyOf<E>]?: E[K] extends Function ? E[K] : (payload: E[K]) => void };

export type TsxComponentAttrs<TProps = {}, TEvents = {}, TScopedSlots = {}> =
    | ({ props: TProps } & Partial<TProps> &
          KnownAttrs & { scopedSlots?: ScopedSlots<TScopedSlots> } & EventHandlers<TEvents> &
          VueTsx.ComponentAdditionalAttrs)
    | (TProps &
          KnownAttrs & { scopedSlots?: ScopedSlots<TScopedSlots> } & EventHandlers<TEvents> &
          VueTsx.ComponentAdditionalAttrs);

export type ElementAttrs<T> = T & KnownAttrs & EventHandlers<dom.EventsOn> & VueTsx.ElementAdditionalAttrs;
